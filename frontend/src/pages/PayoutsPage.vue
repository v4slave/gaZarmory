...
<template>
  <section>
    ...
    <div class="payout-stats"><StatCard label="Золото на расплит" :value="Number(data.summary.gold).toLocaleString('ru-RU')" gold accent/><StatCard label="Праймы" :value="data.summary.primes"/><StatCard label="Участников" :value="data.summary.participants"/><StatCard label="Выплачено за всё время" :value="Number(data.summary.paid_gold ?? 0).toLocaleString('ru-RU')" gold/></div>
    ...
    <div v-if="showForm" class="modal" @click.self="showForm=false"><form class="form-card payout-preview-card" @submit.prevent="create"><h2>Новая платёжная ведомость</h2><div class="payout-mode"><button type="button" :class="{active:selectionMode==='period'}" @click="selectionMode='period';loadPreview()">По периоду</button><button type="button" :class="{active:selectionMode==='activities'}" @click="selectionMode='activities'">Конкретные активности</button></div><template v-if="selectionMode==='period'"><label>Период с<input v-model="from" type="date" required @change="loadPreview"></label><label>Период по<input v-model="to" type="date" :min="from" required @change="loadPreview"></label></template><div v-else class="payout-activity-options"><label v-for="item in preview?.activity_options??[]" :key="item.id"><input v-model="selectedActivities" type="checkbox" :value="item.id" @change="loadPreview"><span><strong>{{ item.definition?.name }}</strong><small>{{ new Date(item.occurred_at).toLocaleString('ru-RU') }}</small></span></label></div><div v-if="previewLoading" class="preview-loading">Рассчитываем выплату…</div><template v-else-if="preview"><div class="financial-preview"><div><span>Активностей</span><strong>{{ preview.activities }}</strong></div><div><span>Игроков</span><strong>{{ preview.players }}</strong></div><div><span>К выдаче</span><strong><GoldAmount :value="Number(preview.amount).toLocaleString('ru-RU')"/></strong></div><div><span>Баланс до</span><strong><GoldAmount :value="Number(preview.balance_before).toLocaleString('ru-RU')"/></strong></div><div><span>Баланс после</span><strong :class="{negative:!preview.sufficient}"><GoldAmount :value="Number(preview.balance_after).toLocaleString('ru-RU')"/></strong></div></div><div class="payout-preview-players"><span v-for="row in preview.rows" :key="row.player_id"><b>{{ row.nickname }}</b><small>{{ row.activities_count }} акт. · {{ Number(row.amount).toLocaleString('ru-RU') }} зол.</small></span></div></template><p v-if="preview&&!preview.sufficient" class="notice error">В казне недостаточно реального золота.</p><p v-else-if="preview&&preview.amount===0" class="notice">В выборке нет свободных начислений.</p><p class="muted">Создание фиксирует ведомость, но не списывает золото. Фактическая выдача отмечается на следующем экране.</p><p v-if="error" class="notice error">{{ error }}</p><div class="form-actions"><button type="button" @click="showForm=false">Отмена</button><button class="primary" :disabled="busy||previewLoading||!preview?.sufficient||!preview?.amount||(selectionMode==='activities'&&!selectedActivities.length)">{{ busy ? 'Создание…' : 'Создать ведомость' }}</button></div></form></div>
  </section>
</template>

...
<template><section><div class="page-heading"><div><p class="eyebrow">GAZ ARMORY · ГИЛЬДИЯ</p><h1>Конст-пати</h1><p class="muted">Каждый игрок может состоять максимум в одной конст-пати</p></div></div>
  ...
  <div class="group-grid"><article v-for="group in guild.groups" :key="group.id" class="group-card"><div class="group-card-heading"><h2>{{ group.name }}</h2><p>{{ group.players_count }} игроков</p></div><div v-if="group.players?.length" class="group-members"><RouterLink v-for="player in sortedPlayers(group)" :key="player.id" :to="`/players/${player.id}`"><strong><span v-if="isPartyLeader(player)" class="party-crown" title="PL · лидер конст-пати">♛</span>{{ player.nickname }}</strong><span class="group-member-meta"><span :class="['class-tag',`class-${player.class}`]">{{ classLabels[player.class] }}</span><small title="Посещённые праймы и мини-праймы">{{ player.primes_count ?? 0 }} / {{ player.mini_activities_count ?? 0 }}</small></span></RouterLink></div><p v-else class="group-empty">В конст-пати пока никого нет</p><div v-if="canManageGroup(group)" class="card-actions"><button @click="rename(group)">Переименовать</button><button class="danger" @click="remove(group)">Удалить</button></div></article>
    <article class="group-card solo"><div class="group-card-heading"><h2>Сольники</h2><p>{{ soloPlayers.length }} игроков</p></div><div v-if="soloPlayers.length" class="group-members"><RouterLink v-for="player in soloPlayers" :key="player.id" :to="`/players/${player.id}`"><strong>{{ player.nickname }}</strong><span class="group-member-meta"><span :class="['class-tag',`class-${player.class}`]">{{ classLabels[player.class] }}</span><small title="Посещённые праймы и мини-праймы">{{ player.primes_count ?? 0 }} / {{ player.mini_activities_count ?? 0 }}</small></span></RouterLink></div><p v-else class="group-empty">Активных сольников нет</p></article>
  </div>
</section></template>

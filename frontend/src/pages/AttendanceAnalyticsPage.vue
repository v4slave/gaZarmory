...
<template><section>
  ...
  <template v-else>
    ...
    <div class="panel attendance-players"><div class="panel-title"><div><h2>Игроки</h2><p class="muted">Процент всегда сопровождается фактическим количеством посещений</p></div></div><div class="table-wrap flat"><table><thead><tr><th>Игрок</th><th>Конста</th><th>Посещаемость</th><th>Текущая серия</th><th>Последний прайм</th></tr></thead><tbody><tr v-for="player in data.players" :key="player.id"><td><RouterLink class="player-identity" :to="`/players/${player.id}`"><PlayerAvatar :player="player" size="tiny"/><span><strong>{{ player.nickname }}</strong><small>{{ classLabels[player.class] }}</small></span></RouterLink></td><td>{{ player.group_name }}</td><td><strong>{{ player.percentage }}%</strong><small class="attendance-fraction">Посещено {{ player.attended }} из {{ player.available }}</small></td><td><span v-if="player.attendance_streak" class="streak-good">✓ {{ player.attendance_streak }} подряд</span><span v-else-if="player.absence_streak" class="streak-bad">× {{ player.absence_streak }} пропуск.</span><span v-else>—</span></td><td>{{ date(player.last_attended_at) }}</td></tr></tbody></table></div></div>

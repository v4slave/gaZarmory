...
const days = computed(() => {
  ...
  return [...grouped.values()].slice(0, 7).map(day => ({
    ...day,
    aglEvents: day.events.filter(event => event.name === 'АГЛ'),
    events: day.events.filter(event => event.name !== 'АГЛ'),
  }))
})
    ...
    <div class="developer-main">
      ...
      <template v-else>
        <section class="developer-calendar">
          <article v-for="day in days" :key="day.key" class="developer-day">
            <header><b>{{ formatDay(day.date) }}</b><small>{{ formatEventDate(day.date) }}</small></header>
            <section v-if="day.aglEvents.length" class="developer-agl">
              <header>
                <img v-if="day.aglEvents[0].icon_url" :src="day.aglEvents[0].icon_url" alt="АГЛ">
                <span v-else>◆</span>
                <div><strong>АГЛ</strong><small>Каждые 4 часа</small></div>
              </header>
              <div>
                <component
...
.developer-agl{margin-top:8px;padding:8px;border:1px solid rgba(193,139,57,.42);background:linear-gradient(145deg,rgba(48,35,18,.78),rgba(8,8,8,.95));box-shadow:inset 0 1px rgba(255,220,154,.04)}.developer-agl>header{display:grid;grid-template-columns:34px 1fr;align-items:center;gap:8px}.developer-agl>header img,.developer-agl>header>span{width:34px;height:34px;object-fit:cover;border:1px solid rgba(217,154,62,.45)}.developer-agl>header>span{display:grid;place-items:center;color:#d9a24b}.developer-agl>header strong,.developer-agl>header small{display:block}.developer-agl>header strong{font-size:13px}.developer-agl>header small{margin-top:2px;color:#9c8d79;font-size:9px;text-transform:uppercase;letter-spacing:.05em}.developer-agl>div{display:grid;grid-template-columns:repeat(3,1fr);gap:4px;margin-top:8px}.developer-agl>div>*{padding:5px 2px;color:#8f8270;text-align:center;text-decoration:none;border:1px solid rgba(157,122,72,.2);background:rgba(0,0,0,.42);font:600 10px ui-monospace,monospace;transition:.15s ease}.developer-agl>div>.linked{color:#a99a84;cursor:pointer}.developer-agl>div>.linked:hover{color:#edbd70;border-color:rgba(217,154,62,.6);background:rgba(67,43,18,.48)}.developer-agl>div>.active,.developer-agl>div>.active:hover{color:#181007;border-color:#f1bd66;background:#e5a94f;box-shadow:0 0 12px rgba(229,169,79,.24)}
</style>

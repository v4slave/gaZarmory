    ...
    <div class="developer-main">
      ...
      <template v-else>
        <section class="developer-calendar">
          <article v-for="day in days" :key="day.key" class="developer-day"><header><b>{{ formatDay(day.date) }}</b><small>{{ formatEventDate(day.date) }}</small></header><RouterLink v-for="event in day.events" :key="`${event.name}-${event.starts_at}`" :to="`/activities/${event.id}`" :title="t('Открыть событие')" :class="['developer-event', { active: isNearestEvent(event) }]"><img v-if="event.icon_url" :src="event.icon_url" :alt="event.name"><span v-else>◆</span><strong>{{ event.name }}</strong><time>{{ formatEventTime(new Date(event.starts_at)) }}</time></RouterLink></article>
        </section>
        <div class="developer-after">
          <section class="developer-selected"><template v-if="nearestEvent"><img v-if="nearestEvent.icon_url" :src="nearestEvent.icon_url" :alt="nearestEvent.name"><div><p>{{ t('БЛИЖАЙШЕЕ СОБЫТИЕ') }}</p><h2>{{ nearestEvent.name }}</h2><span>{{ formatEventDateTime(new Date(nearestEvent.starts_at)) }}</span><RouterLink :to="`/activities/${nearestEvent.id}`">{{ t('Открыть событие') }} →</RouterLink></div></template><p v-else class="developer-empty">{{ t('Ближайших событий нет.') }}</p></section>
...
.developer-heading p{margin:5px 0 0;font-size:11px}.developer-day header small{color:#a89c8c;font-size:11px}.developer-event{text-decoration:none;transition:.16s ease}.developer-event:hover{border-color:#d99a3e;transform:translateY(-1px);box-shadow:0 6px 16px rgba(0,0,0,.3)}.gear-calculator-copy small,.developer-ticker>div>span{font-size:10px}.gear-calculator-copy>span{color:#aaa092;font-size:12px}.developer-ticker small{color:#b2a696}
</style>

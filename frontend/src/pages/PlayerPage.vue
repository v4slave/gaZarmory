...
const auth = useAuthStore(); const showEditor = ref(false); const savingProfile = ref(false); const importingGear = ref(false); const nickname = ref(''); const selectedClass = ref(''); const gearScore = ref(0); const archaGearUrl = ref('')
...
const activitiesPage = ref(1); const activitiesPages = ref(1); const activitiesLoading = ref(false)
const statistics = computed(() => player.value?.statistics ?? {})
const earnings = ref([])
const earningsPage = ref(1); const earningsPages = ref(1); const earningsLoading = ref(false)
...
const gearBySlot = computed(() => Object.fromEntries((player.value?.archa_gear_items ?? []).map(item => [item.slot, item])))
const armorSlots = ['Голова','Нагрудник','Пояс','Наручи','Перчатки','Плащ','Поножи','Обувь','Бельё']
...
async function loadEarnings(id, page = 1) { earningsLoading.value = true; try { const { data } = await api.get(`/api/players/${id}/earnings`, { params: { page, per_page: 10 } }); earnings.value = data.data; earningsPage.value = data.current_page; earningsPages.value = data.last_page } finally { earningsLoading.value = false } }
...
async function importArchaGear() {
  importingGear.value = true; error.value = ''
  try {
    const endpoint = !isOwnProfile.value && isDeveloper.value ? `/api/players/${player.value.id}/archa-gear` : '/api/me/player/archa-gear'
    await api.post(endpoint, { archa_gear_url: archaGearUrl.value })
    player.value = (await api.get(`/api/players/${route.params.id}`)).data
    syncEditor()
    notifications.success('Экипировка с archa.ge импортирована.')
  } catch (e) { error.value = apiErrorMessage(e, 'Не удалось импортировать экипировку.'); notifications.error(error.value) }
  finally { importingGear.value = false }
}
</script>

...
const typeLabels={prime:'Основной прайм',mini_activity:'Мини-прайм'}
function status(item){if(item.definition.type==='mini_activity'&&item.completed_at)return 'Завершена';if(item.earnings_count>0)return 'Рассчитана';return 'Черновик'}
watch(()=>[activities.filters.type,activities.filters.definition_id,activities.filters.date_from,activities.filters.date_to],activities.fetchActivities)
onMounted(()=>Promise.all([activities.fetchDefinitions(),activities.fetchActivities()]))
</script>
<template><section><div class="page-heading"><div><p class="eyebrow">GAZ ARMORY · ЖУРНАЛ</p><h1>Активности</h1><p class="muted">Основные и мини-праймы гильдии</p></div><button v-if="auth.canManage" class="primary" @click="showForm=true">Создать событие</button></div>
  <div class="toolbar activity-toolbar"><select v-model="activities.filters.definition_id"><option value="">Все события</option><option v-for="definition in activities.definitions" :key="definition.id" :value="definition.id">{{ definition.name }}</option></select><select v-model="activities.filters.type"><option value="">Все типы</option><option value="prime">Основные праймы</option><option value="mini_activity">Мини-праймы</option></select><label><span>С даты</span><input v-model="activities.filters.date_from" type="date"></label><label><span>По дату</span><input v-model="activities.filters.date_to" type="date"></label></div>
  ...
  <div v-if="showForm" class="modal" @click.self="showForm=false"><ActivityForm @created="activities.fetchActivities()" @cancel="showForm=false" /></div>
</section></template>

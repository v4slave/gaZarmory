...
watch(()=>[activities.filters.type,activities.filters.date_from,activities.filters.date_to],activities.fetchActivities)
...
<template><section><div class="page-heading"><div><p class="eyebrow">GAZ ARMORY · ЖУРНАЛ</p><h1>Активности</h1><p class="muted">Основные и мини-праймы гильдии</p></div><button v-if="auth.canManage" class="primary" @click="showForm=true">Создать событие</button></div>
  <div class="toolbar"><select v-model="activities.filters.type"><option value="">Все типы</option><option value="prime">Основные праймы</option><option value="mini_activity">Мини-праймы</option></select><input v-model="activities.filters.date_from" type="date"><input v-model="activities.filters.date_to" type="date"></div>

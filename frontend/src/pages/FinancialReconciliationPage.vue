...
const checkedAt=computed(()=>formatDateTime(report.value.checked_at))
...
<template><section>
  ...
  <template v-else>
    ...
    <div class="reconciliation-grid"><article v-for="check in report.checks" :key="check.key" :class="['panel','reconciliation-check',`status-${check.status}`]"><div class="reconciliation-check-heading"><span>{{ statusIcons[check.status] }}</span><div><h2>{{ check.title }}</h2><p>{{ check.description }}</p></div><b>{{ check.issues_count }}</b></div><div v-if="Object.keys(check.metrics).length" class="reconciliation-metrics"><span v-for="(value,key) in check.metrics" :key="key"><small>{{ metricsLabels[key]??key }}</small><b>{{ formatInteger(value) }}</b></span></div><div v-if="check.issues.length" class="reconciliation-issues"><article v-for="(issue,index) in check.issues" :key="`${issue.entityType}-${issue.entityId}-${index}`" :class="issue.severity"><span>{{ issue.severity==='critical'?'×':'!' }}</span><div><strong>{{ issue.title }}</strong><small>{{ issue.details }}</small></div><RouterLink v-if="entityLink(issue)" :to="entityLink(issue)">Открыть →</RouterLink></article></div><p v-else class="reconciliation-ok">✓ Проверка пройдена, расхождений не найдено</p></article></div>
  </template>
</section></template>

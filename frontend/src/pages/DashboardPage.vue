...
<template>
  <section>
    ...
    <div class="stats-row four"><StatCard label="Золото в казне" :value="`${Number(data.gold).toLocaleString('ru-RU')} 🪙`" note="Фактический баланс" accent/><StatCard label="Дроп с РБ" :value="`${Number(data.inventory_value).toLocaleString('ru-RU')} 🪙`" note="Эквивалент в золоте"/><StatCard label="Ожидаемый нахрюк" :value="`${Number(data.pending_payout).toLocaleString('ru-RU')} 🪙`"/><StatCard label="Активных аукционов" :value="data.active_auctions"/></div>

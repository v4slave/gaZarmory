...
function compactGold(value) {
  ...
  if (!unit) return formatInteger(Math.round(amount))
  const labels = getLocale() === 'en'
    ? { 'квадр.': 'qd', 'трлн': 'tn', 'млрд': 'bn', 'млн': 'm', 'тыс.': 'k' }
    : {}
  return `${formatDecimal(amount / unit[0], { maximumFractionDigits: 1 })} ${labels[unit[1]] ?? unit[1]}`
}
function eventCountdown(startsAt) {
  const seconds=Math.max(0,Math.floor((new Date(startsAt).getTime()-now.value)/1000));
  if(seconds<60)return `${seconds} ${getLocale()==='en'?'sec.':'сек.'}`
  const minutes=Math.floor(seconds/60); if(minutes<60)return `${minutes} ${getLocale()==='en'?'min.':'мин.'}`
  const hours=Math.floor(minutes/60); const rest=minutes%60
  if(getLocale()==='en')return hours<24?`${hours} hr. ${rest} min.`:`${Math.floor(hours/24)} d. ${hours%24} hr.`

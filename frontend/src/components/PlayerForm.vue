...
const error = ref('')
const form = reactive({ nickname: '', class: 'melee', group_id: auth.isPartyLeader && !auth.canManage ? auth.partyGroupId : null, is_active: true })
const classes = [['melee', 'Милик'], ['archer', 'Лучник'], ['mage', 'Маг'], ['healer', 'Хил'], ['bard', 'Бард'], ['tank', 'Танк']]
const availableGroups = computed(() => auth.isPartyLeader && !auth.canManage
  ? guild.groups.filter(group => group.id === auth.partyGroupId)
  : guild.groups)
...
<template>
  <form class="form-card" @submit.prevent="submit">
    ...
    <label>Конст-пати<select v-model="form.group_id" :disabled="auth.isPartyLeader && !auth.canManage"><option v-if="auth.canManage" :value="null">Сольники</option><option v-for="group in availableGroups" :key="group.id" :value="group.id">{{ group.name }}</option></select></label>

...
**Micro Guild Leader:** управляет гильдией и заявками на привязку, но не
создаёт аукционы и выплаты, не продаёт и не выдаёт предметы казны, не
назначает Guild Leader или Developer.

**Developer:** полный доступ наравне с Guild Leader.

**Party Leader:** управляет только составом собственной конст-пати. Не
может создавать, переименовывать или удалять группы.

**Member:** просмотр состава, профилей, активностей, всей истории казны,
аукционов и собственных выплат; может делать ставки.
...
оценочная стоимость дропа в золоте, рассчитывается доля каждого участника
и создается ожидаемая выплата. Эта оценка не увеличивает фактический
золотой баланс казны.

**Activity** --- учет события, посещаемости и редкого лута. Само
посещение не создает денежное начисление.

**Mini Activity** --- дополнительная активность с учетом посещаемости и
дропа. Оценочная стоимость полученного лута равномерно распределяется
между фактическими участниками так же, как для Prime.
...
Остаток оценочной стоимости не распределяется. Нельзя начислить больше
`gold_value`. Оценочная стоимость лута не является золотом казны.
...
По умолчанию сумма рассчитывается автоматически. При создании ведомости ГЛ или Разработчик может задать произвольную общую сумму в золоте или жетонах; она распределяется пропорционально исходным начислениям игроков, а курс жетона фиксируется на момент создания.
...
Показывает: - фактическое золото; - стоимость инвентаря в эквиваленте
золота; - инвентарь; - последний дроп; - историю выдачи предметов; -
финансовые транзакции.

Золото и оценочную стоимость предметов отображать отдельно. Дроп
увеличивает только количество и оценочную стоимость предметов. Золотой
баланс увеличивается только при аукционной или внешней продаже.

...
final class PayPayoutPlayers
{
    ...
    public function execute(Payout $payout, array $playerIds, int $userId): Payout
    {
        try { return DB::transaction(function () use ($payout,$playerIds,$userId): Payout {
            ...
            foreach($rows as $row){$discordId=$row->player?->user?->discord_id;if(!$discordId)continue;$details=PrimePlayerEarning::query()->where('payout_id',$locked->id)->where('player_id',$row->player_id)->with('activity.definition:id,name')->get()->groupBy(fn($earning)=>$earning->activity?->definition?->name??'Активность')->map(fn($items,$name)=>'• '.$name.': **'.number_format($items->sum('player_share'),0,'',' ').'**')->implode("\n");DB::afterCommit(fn()=>SendPlayerPayoutNotification::dispatch((string)$discordId,'Выплата по нахрюку #'.$locked->id,'Вам выдано **'.number_format($row->amount,0,'',' ').' золота**.'.($details?"\n\nРасшифровка:\n".$details:'')));}

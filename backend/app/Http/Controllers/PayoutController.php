...
final class PayoutController extends Controller
{
    ...
    public function store(Request $request,AuditService $audit,CalculatePayout $calculate,CompletePayout $complete){abort_unless($request->user()->canManageGuild(),403);$data=$request->validate(['period_from'=>['required','date'],'period_to'=>['required','date','after_or_equal:period_from']]);return DB::transaction(function()use($request,$data,$audit,$calculate,$complete){$payout=Payout::query()->create($data+['status'=>'draft','created_by'=>$request->user()->id]);$audit->record('payout.created',$payout,null,$payout->getAttributes());$calculated=$calculate->execute($payout);$paid=$complete->execute($calculated,$request->user()->id);return response()->json($paid,201);});}

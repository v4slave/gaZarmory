...
final class AuctionController extends Controller
{
    ...
    public function store(Request $request,AuditService $audit)
    {
        ...
        $data=$request->validate(['treasury_item_id'=>['required','integer','exists:treasury_items,id'],'quantity'=>['required','integer','min:1'],'starting_bid'=>['required','integer','min:0'],'minimum_step'=>['required','integer','min:1'],'extension_minutes'=>['sometimes','integer','min:2','max:5'],'ends_at'=>['required','date','after_or_equal:'.now()->addMinutes(10)->toISOString()]],['ends_at.after_or_equal'=>__('domain.auction.minimum_duration')]);$data['extension_minutes']??=3;
        return DB::transaction(function()use($request,$data,$audit){$item=TreasuryItem::query()->lockForUpdate()->findOrFail($data['treasury_item_id']);if($data['quantity']>$item->available_quantity)throw ValidationException::withMessages(['quantity'=>__('domain.auction.insufficient_stock')]);$auction=Auction::query()->create($data+['status'=>'draft','created_by'=>$request->user()->id]);$audit->record('auction.created',$auction,null,$auction->getAttributes());return response()->json($auction->load('item'),201);});
    }
    public function update(Request $request,Auction $auction,AuditService $audit)
    {
        abort_unless($request->user()->canCreateAuctions(),403);
        $data=$request->validate(['treasury_item_id'=>['required','integer','exists:treasury_items,id'],'quantity'=>['required','integer','min:1'],'starting_bid'=>['required','integer','min:0'],'minimum_step'=>['required','integer','min:1'],'extension_minutes'=>['sometimes','integer','min:2','max:5'],'ends_at'=>['required','date','after_or_equal:'.now()->addMinutes(10)->toISOString()]],['ends_at.after_or_equal'=>__('domain.auction.minimum_duration')]);$data['extension_minutes']??=$auction->extension_minutes;
        return DB::transaction(function()use($auction,$data,$audit){$locked=Auction::query()->lockForUpdate()->findOrFail($auction->id);if($locked->status!=='draft')throw ValidationException::withMessages(['auction'=>__('domain.auction.edit_draft_only')]);$item=TreasuryItem::query()->lockForUpdate()->findOrFail($data['treasury_item_id']);if($data['quantity']>$item->available_quantity)throw ValidationException::withMessages(['quantity'=>__('domain.auction.insufficient_stock')]);$old=$locked->only(array_keys($data));$locked->update($data);$audit->record('auction.updated',$locked,$old,$data);return $locked->refresh()->load('item');});
    }
    public function start(Request $request,Auction $auction,AuditService $audit,ArmoryNotificationService $notifications)
    {
        abort_unless($request->user()->canCreateAuctions(),403);
        return DB::transaction(function()use($auction,$audit,$notifications){$locked=Auction::query()->lockForUpdate()->findOrFail($auction->id);if($locked->status!=='draft')throw ValidationException::withMessages(['auction'=>__('domain.auction.start_draft_only')]);if($locked->ends_at->lt(now()->addMinutes(10)))throw ValidationException::withMessages(['ends_at'=>__('domain.auction.minimum_remaining_time')]);$tokenUnitValue=(int)(DB::table('treasury_token_settings')->where('id',1)->lockForUpdate()->value('token_unit_value')??0);if($tokenUnitValue<=0)throw ValidationException::withMessages(['auction'=>__('domain.auction.token_value_required')]);$item=TreasuryItem::query()->lockForUpdate()->findOrFail($locked->treasury_item_id);if($locked->quantity>$item->available_quantity)throw ValidationException::withMessages(['quantity'=>__('domain.auction.insufficient_stock')]);$item->increment('reserved_quantity',$locked->quantity);$locked->update(['status'=>'active','token_unit_value_snapshot'=>$tokenUnitValue]);$audit->record('auction.started',$locked,['status'=>'draft'],['status'=>'active','token_unit_value_snapshot'=>$tokenUnitValue]);DB::afterCommit(function()use($locked,$item,$notifications){SendDiscordNotification::dispatch('Новый аукцион','Выставлен лот «'.$item->item_name.'» ×'.$locked->quantity.'. Стартовая ставка: **'.number_format($locked->starting_bid,0,'',' ').' жетонов**. Завершение: '.$locked->ends_at->format('d.m.Y H:i').'.','gold','auctions');$notifications->notify($notifications->activeMembers(),'auction_started','Аукцион начался','Лот «'.$item->item_name.'» ×'.$locked->quantity.' открыт для ставок.','/auctions/'.$locked->id,'auction-started-'.$locked->id);});return $locked->refresh()->load('item');});
    }
    public function bid(Request $request,Auction $auction,PlaceAuctionBid $action){$player=$request->user()->player;abort_unless($player&&$player->is_active,403,__('domain.auction.active_player_required'));$amount=$request->validate(['amount'=>['required','integer','min:0']])['amount'];return response()->json($action->execute($auction,$player,$amount),201);}

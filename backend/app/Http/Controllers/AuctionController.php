...
final class AuctionController extends Controller
{
    ...
    {
        abort_unless($request->user()->canManageGuild(),403);
        return DB::transaction(function()use($auction,$audit,$notifications){$locked=Auction::query()->lockForUpdate()->findOrFail($auction->id);if($locked->status!=='draft')throw ValidationException::withMessages(['auction'=>'Запустить можно только черновик.']);if($locked->ends_at->lt(now()->addMinutes(10)))throw ValidationException::withMessages(['ends_at'=>'До завершения аукциона должно оставаться не менее 10 минут.']);$item=TreasuryItem::query()->lockForUpdate()->findOrFail($locked->treasury_item_id);if($locked->quantity>$item->available_quantity)throw ValidationException::withMessages(['quantity'=>'Недостаточно свободного количества в казне.']);$item->increment('reserved_quantity',$locked->quantity);$locked->update(['status'=>'active']);$audit->record('auction.started',$locked,['status'=>'draft'],['status'=>'active']);DB::afterCommit(function()use($locked,$item,$notifications){SendDiscordNotification::dispatch('Новый аукцион','Выставлен лот «'.$item->item_name.'» ×'.$locked->quantity.'. Стартовая ставка: **'.number_format($locked->starting_bid,0,'',' ').' золота**. Завершение: '.$locked->ends_at->format('d.m.Y H:i').'.');$notifications->notify($notifications->activeMembers(),'auction_started','Аукцион начался','Лот «'.$item->item_name.'» ×'.$locked->quantity.' открыт для ставок.','/auctions/'.$locked->id,'auction-started-'.$locked->id);});return $locked->refresh()->load('item');});
    }

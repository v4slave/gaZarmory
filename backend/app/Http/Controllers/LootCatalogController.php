...
final class LootCatalogController extends Controller
{
    ...
    public function update(Request $request,LootCatalogItem $lootCatalogItem,AuditService $audit){abort_unless($request->user()->canManageGuild(),403);$data=$request->validate(['name'=>['sometimes','required','string','max:255',Rule::unique('loot_catalog_items','name')->ignore($lootCatalogItem)],'icon'=>['nullable','image','mimes:png,jpg,jpeg,webp,gif','max:2048']]);$old=$lootCatalogItem->getAttributes();if(isset($data['name']))$lootCatalogItem->name=$data['name'];if($request->file('icon'))$lootCatalogItem->icon_path=$request->file('icon')->store('loot-catalog','public');$lootCatalogItem->save();$audit->record('loot_catalog.updated',$lootCatalogItem,$old,$lootCatalogItem->getAttributes());return $lootCatalogItem->refresh();}
    public function destroy(Request $request,LootCatalogItem $lootCatalogItem,AuditService $audit){abort_unless($request->user()->canManageGuild(),403);$lootCatalogItem->update(['is_active'=>false]);$audit->record('loot_catalog.deactivated',$lootCatalogItem,['is_active'=>true],['is_active'=>false]);return response()->noContent();}
}

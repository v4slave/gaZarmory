...
final class LootCatalogController extends Controller
{
    ...
    public function store(Request $request,AuditService $audit)
    {
        ...
        if($existing?->is_active)throw \Illuminate\Validation\ValidationException::withMessages(['name'=>__('domain.loot.catalog_duplicate')]);
    ...
    public function destroy(Request $request,LootCatalogItem $lootCatalogItem,AuditService $audit){abort_unless($request->user()->canAdministrate(),403);$data=$request->validate(['updated_at'=>['nullable','date']]);if(isset($data['updated_at'])&&!$lootCatalogItem->updated_at->equalTo($data['updated_at']))throw \Illuminate\Validation\ValidationException::withMessages(['updated_at'=>__('domain.loot.catalog_stale')]);$lootCatalogItem->update(['is_active'=>false]);$audit->record('loot_catalog.deactivated',$lootCatalogItem,['is_active'=>true],['is_active'=>false]);return response()->noContent();}
}

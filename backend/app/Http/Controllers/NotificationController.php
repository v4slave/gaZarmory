...
{
 public function index(Request $request){$query=ArmoryNotification::query()->where('user_id',$request->user()->id);return response()->json(['unread_count'=>(clone $query)->whereNull('read_at')->count(),'items'=>$query->latest()->limit(50)->get()]);}
